...
app.secret_key = 'key'
app.config[
    'SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://sql8697943:2HSgWDVuLg@sql8.freemysqlhosting.net/sql8697943'
db = SQLAlchemy(app)
...
def login_route():
  if request.method == 'POST':
    ...
    user = accounts.query.filter_by(username=username,
                                    pasword=password).first()
    if user:
...
def quiz_route():
  ...
  if 'user_id' in session:
    ...
    if first_quiz:
      ...
      taken_quiz_record = taken_quiz.query.filter_by(user_id=user_id,
                                                     quiz_id=quiz_id).first()
      if taken_quiz_record:
        taken_quiz_record.score = 0
        db.session.commit()
      else:
        taken_quiz_record = taken_quiz(user_id=user_id,
                                       quiz_id=quiz_id,
                                       score=0)
...
def quiz_history():
  if 'user_id' in session:
    ...
    subquery = db.session.query(
        taken_quiz.quiz_id,
        func.max(taken_quiz.taken_id).label('max_taken_id')).group_by(
            taken_quiz.quiz_id).subquery()
    user_quizzes = db.session.query(taken_quiz).join(
        subquery, taken_quiz.taken_id == subquery.c.max_taken_id).filter(
            taken_quiz.user_id == user_id).all()
...
def quiz_questions(quiz_id, question_number):
  ...
  if request.method == 'POST':
    ...
    if selected_answer and selected_answer.correct:
      if 'user_id' in session:
        user_id = session['user_id']
        answered_correctly = session.get('answered_correctly', [])
        if current_question.question_id not in answered_correctly:
          answered_correctly.append(current_question.question_id)
          session['answered_correctly'] = answered_correctly

          #check if the user has answered two consecutive questions correctly
          if len(answered_correctly) % 2 == 0:
            #increment difficulty level for every two consecutive correct answers
            current_question.difficulty_level += 1
            db.session.commit()

            #reset the answered_correctly list to start counting again
            session.pop('answered_correctly', None)

        #update the user's score in the database
          taken_quiz_record = taken_quiz.query.filter_by(
    ...
    next_question = question.query.filter(
        ...
        question.qnumber > question_number,
        question.difficulty_level >= current_question.difficulty_level
    ).order_by(question.qnumber).first()

    next_question_number = question_number + 1
    next_question = question.query.filter_by(
        quiz_id=quiz_id, qnumber=next_question_number).first()
    if next_question:
      return redirect(
          url_for('quiz_questions',
                  quiz_id=quiz_id,
                  question_number=next_question_number))
    else:
      return redirect(url_for('quiz_summary', quiz_id=quiz_id))

  return render_template('chambersq.html',
